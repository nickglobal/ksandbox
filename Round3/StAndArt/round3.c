...
#define INTERVAL_TIME 1000 // 1 sec 
...
struct ssd1306_data *ssd1306 = NULL;
struct class *pt_class = NULL;
static struct workqueue_struct *pt_wq = NULL;
u32 ui_time = 0;
...
static void handle_function(struct work_struct *pt_work)
{
    char pc_time[20];

    sprintf(pc_time, "%12u", ui_time);
    ssd1306_GotoXY(ssd1306, 8, 25);
    ssd1306_Puts(ssd1306, pc_time, &TM_Font_7x10, SSD1306_COLOR_WHITE);
...
static int add_work_to_q(void)
{
    struct delayed_work *pt_work = NULL;
    int ret = 0;

    if (NULL != pt_wq && ui_time)
    {
        pt_work = (struct delayed_work*) kzalloc(sizeof(struct delayed_work), GFP_KERNEL);
        if (NULL == pt_work)
        {
        ...
        // make work
        INIT_DELAYED_WORK(pt_work, handle_function);

        // add it to queue with 1 sec delay
        ret = queue_delayed_work(pt_wq, pt_work, msecs_to_jiffies(INTERVAL_TIME));
        if (false == ret)
        {
           printk("ERROR: queue_delayed_work() return %d\n", ret);
           return -EINVAL;
        }
    }
...
static ssize_t sys_class_init_time_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
    int ret = 0;
    u32 ui_init_time = 0;

    ret = sscanf(buf, "%u", &ui_init_time);
    if (ret != 1)
    {
    ...
    ui_time = ui_init_time;

    add_work_to_q(); // start timing functionality
...
static void clear_global_data(void)
{
    ...
    class_remove_file(pt_class, &t_class_attrs);
...
ssd1306_probe(struct i2c_client *drv_client, const struct i2c_device_id *id)
{
    ...
    ssd1306_GotoXY(ssd1306, 8, 25);
    ssd1306_Puts(ssd1306, "SET TIME PLS", &TM_Font_7x10, SSD1306_COLOR_WHITE);
    //ssd1306_Puts(ssd1306, ":", &TM_Font_7x10, SSD1306_COLOR_WHITE);
    ssd1306_UpdateScreen(ssd1306);

    pt_class = class_create(THIS_MODULE, "TIMER");
    if (IS_ERR(pt_class))
    {
        printk("ERROR: class_create() ERROR\n");
        return PTR_ERR(pt_class);
    }

    // set sys class attributes
    t_class_attrs.attr.name = "init-time";
    t_class_attrs.attr.mode = 0664;
    t_class_attrs.show = NULL;
    t_class_attrs.store = &sys_class_init_time_store;

    ret = class_create_file(pt_class, &t_class_attrs);
    if (ret < 0)
    {
        printk("ERROR: class_create_file(): ret %d\n", ret);
        clear_global_data();
        return ret;
    }

    // timer/tasklet don't work here, use workqueue
    pt_wq = create_workqueue("timer_queue");
    if (NULL == pt_wq)
    {
        printk("ERROR: cannot create workqueue\n");
        clear_global_data();
        return -EINVAL;
    }
