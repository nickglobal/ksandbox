...
#define DRIVER_AUTHOR  "GL team + SanB (O.Bobro)"
#define DRIVER_DESC "ssd1306_drv_SanB.ko driver with EXPORT"
#define DEVICE_DRIVER_NAME "ssd1306_drv_SanB"
#define BUS_NAME    "i2c_2"
...
{
	u8      type;
    u8      data[SSD1306_WIDTH * SSD1306_HEIGHT / 8];
...
int ssd1306_print_XYZ(void *ssd1306, int *vals, int count, int print_accel_only_flag)
{
	int i;
	char str[19];

//	printk(KERN_ERR "ssd1306_print_XYZ START!\n");

	if (unlikely(!ssd1306 || !vals))
		return -1;

	memset(&dataArray.data[0], 0, sizeof(dataArray.data));
	dataArray.type = 0x40;
	...
	for (i=0; i<count; i++)
	{
		sprintf(str, "%s = %5d", valNames[i + (print_accel_only_flag ? 3 : 0)], vals[i]);
		ssd1306_GotoXY((struct ssd1306_data *)ssd1306, 10, i*10);
		ssd1306_Puts((struct ssd1306_data *)ssd1306, str, &TM_Font_7x10, SSD1306_COLOR_WHITE);
	}

	i = ssd1306_UpdateScreen((struct ssd1306_data *)ssd1306);
//	printk(KERN_ERR "ssd1306_print_XYZ END=====!\n");
	return i;
}
EXPORT_SYMBOL_GPL(ssd1306_print_XYZ);
...
static void ssd1306_init_lcd(struct i2c_client *drv_client)
{
    ...
    i2c_smbus_write_byte_data(drv_client, 0x00, 0x00);	// 0x00 - Horizontal Addressing Mode;
    ...
    {
    	unsigned char empty[32] = {0x00};
    	for (i = 0; i < 32; i++) {
    		i2c_smbus_write_i2c_block_data(drv_client, 0x40, 32, &empty[0]);
    	}
    }
}
/**/
int ssd1306_UpdateScreen(struct ssd1306_data *drv_data)
{
    ...
    dataArray.type = 0x40;
	ret = i2c_master_send(drv_client, (u8 *) &dataArray, sizeof(dataArray));
	if (unlikely(ret < 0))
	{
...
ssd1306_probe(struct i2c_client *drv_client, const struct i2c_device_id *id)
{
    ...
    printk(KERN_INFO "drv_client = [%p], &drv_client->dev = [%p], ssd1306 = [%p]\n", drv_client, &drv_client->dev, ssd1306);
    ...
    ssd1306_Puts(ssd1306, "READY TO PRINT...", &TM_Font_7x10, SSD1306_COLOR_WHITE);
